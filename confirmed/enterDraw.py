...
async def enter_draw(account, profile, sku, size, delay):
    try:
        ...
        profile["address1"] = random3 + " " + baseAddress
        ...
        # get valid area code
        areaCodes = ["925", "510", "408", "650", "415", "707", "209", "559", "831"]
        profile["phone"] = random.choice(areaCodes) + ranDig() + ranDig() + ranDig() + ranDig()+ ranDig()+ ranDig()+ ranDig()
        ...
        task.setStatus("Adding Draw Product to Cart - " + size + " - " + technicalSize, "warning")
        ...
        if request.status_code != 200:
            task.setStatus("Error Adding Draw Product to Cart! - " + request.json()["detail"], "bad")
            return
        data = request.json()
        ...
        request = (await session.post(f"https://api.3stripes.net/gw-api/v2/hype/basket/{basketId}/order", json_data=submitjson))
        ...
        if "PaymentMethodRefused" in request.text:
            await asyncio.sleep(30)
            task.setStatus("Payment Method Refused, Retrying...", "bad")
        ...
        if request.status_code != 200:
            ...
            task.setStatus("Successfully Entered Draw!", "good")
            webhook = DiscordWebhook(url='')
            embed = DiscordEmbed(title='Entered Draw', description='Your account has been entered for ' + sku + '.', color="2ecc71")
            embed.add_embed_field(name='Site', value="Adidas / Confirmed (DRAW)", inline=False)
